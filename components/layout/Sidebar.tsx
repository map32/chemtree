...
import { CATEGORIES } from '@/config/categories' // Importing your JSON object
    ...
    <nav className="sticky top-8 space-y-2">
      <div className="bg-navy-900 rounded-lg p-4 border border-navy-800 shadow-lg">
        ...
        <ul className="space-y-1">
          ...
          {/* 2. DYNAMIC CATEGORIES (From Config) */}
          {CATEGORIES.map((cat) => {
            const Icon = cat.icon
            ...
            return (
              <li key={cat.id}>
                ...
                >
                  <Icon className={`w-5 h-5 ${isActive ? 'text-navy-950' : 'text-slate-500'}`} />
                  <span>{cat.label}</span>
                </Link>
              </li>
            )
